...
NOMA_MUST_EXIST = [
    ...
    (62, 2009, "神器―軍艦「橿原」殺人事件（上・下）", "奥泉光"),
    (58, 2005, "半島を出よ（上・下）", "村上龍"),
    (47, 1994, "志賀直哉（上・下）", "阿川弘之"),
    (33, 1980, "侍", "遠藤周作"),
    (26, 1973, "洪水はわが魂に及び（上・下）", "大江健三郎"),
]
...
def test_award_books_noma_rounds():
    ...
    第2・4・8・37・48回は受賞作なしのため欠番が正しい状態。"""
    ...
    assert len(rows) == 84, f"レコード数(行単位)が想定と異なります: {len(rows)}件"
...
def test_award_books_yomiuri_novel_rounds():
    ...
    assert rounds == expected_rounds, f"想定と異なる回次構成です: {sorted(rounds)}"
    assert len(rows) == 85, f"レコード数(行単位)が想定と異なります: {len(rows)}件"
